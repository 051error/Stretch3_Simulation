#!/usr/bin/env python3
"""Interactive command-line controller for Stretch 3 via ROS 2."""
...
from stretch_sim.navigation import NavigationController
...
class InteractiveController(Node):
    ...
    def __init__(self, actions_file=None):
        super().__init__('interactive_controller')

        if actions_file is None:
            actions_path = get_config_path("actions.yaml")
        else:
            candidate = Path(actions_file)
            actions_path = candidate if candidate.is_absolute() else get_config_path(actions_file)
