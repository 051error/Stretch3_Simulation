...
# Seconds without an incoming cmd_vel before the base velocity is zeroed, so the
# robot stops when Nav2 or a teleop source stops publishing instead of drifting.
CMD_VEL_TIMEOUT = 0.5

# 2D lidar constants (matches the N rangefinders generated by tools/gen_laser.py).
# 90 rays (4-degree resolution) keeps mj_step near real time; a full 360-ray lidar
# ray-cast dominated mj_step and made the base drive ~7x slower than real time.
NUM_LASER_RAYS = 90
LASER_ANGLE_MIN = -math.pi
LASER_ANGLE_MAX = math.pi
LASER_RANGE_MIN = 0.1   # metres
LASER_RANGE_MAX = 8.0   # metres
# Rays shorter than this are the robot's own mast/base (self-hit); the nearest
# real obstacle in the kitchen scene is ~1.4 m away, so 0.3 m is unambiguous.
SELF_HIT_MIN_DIST = 0.3
LASER_FRAME = 'laser'
BASE_FRAME = 'base_link'
ODOM_FRAME = 'odom'
# Measured base-drive steady-state scaling: forward ctrl=1 -> FORWARD_SCALE m/s,
# turn ctrl=1 -> -TURN_SCALE rad/s (clockwise, i.e. negative ROS angular.z).
# Recalibrated from the steady-state wheel velocity (wheel radius 0.05 m)
# under integrator=implicitfast, timestep=0.005.
FORWARD_SCALE = 0.25
TURN_SCALE = 0.50
...
class StretchSimNode(Node):
    ...
    def _init_laser(self):
        """Locate the lidar rangefinder sensors and their sensordata address."""
        self.laser_adr = -1
        first_id = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_SENSOR, 'laser_0')
        if first_id < 0:
            self.get_logger().warn('Lidar "laser_0" not found, /scan disabled')
            return
        self.laser_adr = self.model.sensor_adr[first_id]
        self.get_logger().info(
            f'Lidar ready: {NUM_LASER_RAYS} rays at sensordata address {self.laser_adr}'
        )
    ...
    def _cmd_vel_callback(self, msg):
        ...
        # Convert m/s and rad/s to motor ctrl (ctrlrange is -1..1).
        self.ctrl_state['forward'] = float(np.clip(linear_x / FORWARD_SCALE, -1.0, 1.0))
        self.ctrl_state['turn'] = float(np.clip(-msg.angular.z / TURN_SCALE, -1.0, 1.0))
    ...
    def publish_joint_states(self, stamp=None):
        ...
        stamp: shared ROS timestamp so all publishers in one loop iteration agree.
        """
        msg = JointState()
    ...
    def _publish_scan(self, stamp=None):
        """Publish a LaserScan built from the lidar rangefinder data."""
        if self.laser_adr < 0:
            return
        msg = LaserScan()
        msg.header.stamp = stamp if stamp is not None else self.get_clock().now().to_msg()
        msg.header.frame_id = LASER_FRAME
        msg.angle_min = LASER_ANGLE_MIN
        msg.angle_max = LASER_ANGLE_MAX
        msg.angle_increment = (LASER_ANGLE_MAX - LASER_ANGLE_MIN) / NUM_LASER_RAYS
        msg.range_min = LASER_RANGE_MIN
        msg.range_max = LASER_RANGE_MAX
        ranges = []
        base = self.laser_adr
        for i in range(NUM_LASER_RAYS):
            d = float(self.data.sensordata[base + i])
            # -1 = no obstacle; below SELF_HIT_MIN_DIST = robot's own mast
            if d < 0.0 or d < SELF_HIT_MIN_DIST:
                d = float('inf')
            ranges.append(min(d, LASER_RANGE_MAX))
        msg.ranges = ranges
        self.scan_pub.publish(msg)

    def _publish_odom_tf(self, stamp=None):
        """Publish the odom -> base_link TF and an Odometry message."""
        ...
        # base velocities from the freejoint qvel (world frame)
    ...
    def run_simulation(self):
        ...
        with mujoco.viewer.launch_passive(
            ...
            prev_render_time = prev_pub_time = prev_camera_time = prev_scan_time = time.time()
            pub_interval = 1.0 / PUB_RATE
            render_interval = 1.0 / RENDER_RATE
            camera_interval = 1.0 / PUB_RATE
            scan_interval = 1.0 / 10.0  # 10 Hz is enough for costmap updates
            
            while self.running and viewer.is_running():
                step_start = time.time()
                
                self._update_navigation()
                if (not self.nav_controller.is_active()
                        and time.time() - self.last_cmd_vel_time > CMD_VEL_TIMEOUT):
                    self.ctrl_state['forward'] = 0.0
                    self.ctrl_state['turn'] = 0.0
                ...
                if now - prev_pub_time > pub_interval:
                    self.publish_joint_states(stamp=ros_now)
                    self._publish_odom_tf(stamp=ros_now)
                    prev_pub_time = now

                if now - prev_scan_time > scan_interval:
                    self._publish_scan(stamp=ros_now)
                    prev_scan_time = now
